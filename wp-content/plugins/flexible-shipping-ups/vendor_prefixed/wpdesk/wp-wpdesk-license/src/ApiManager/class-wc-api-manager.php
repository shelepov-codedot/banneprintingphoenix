...
if (!\class_exists('UpsFreeVendor\\WPDesk_API_Manager_With_Update_Flag')) {
    class WPDesk_API_Manager_With_Update_Flag
    {
         ...
         */
        public $text_domain = 'flexible-shipping-ups';
        /**
         * @var string
         */
        public $plugin_url;
        ...
        private function add_not_possible_update_message()
        {
            \add_action('in_plugin_update_message-' . $this->plugin_name, function (array $plugin_data, \stdClass $response) {
                if (isset($response, $response->package) && empty($response->package)) {
                    echo \sprintf(\__(" <a target='_blank' href='%s'>Enter a valid subscription key for automatic updates.</a>", 'flexible-shipping-ups'), \admin_url('admin.php?page=wpdesk-licenses'));
                }
        ...
        {
            $parser = new \UpsFreeVendor\WPDesk\License\Changelog\Parser($changelog);
            $parser->parse();
            $parsed_changelog = $parser->get_parsed_changelog()->getIterator();
            $changes = new \UpsFreeVendor\WPDesk\License\Changelog\Filter\ByVersion($parsed_changelog, $plugin_version);
            if (\iterator_count($changes) > 0) {
                $changelog = new \UpsFreeVendor\WPDesk\License\Changelog\Formatter($changes);
