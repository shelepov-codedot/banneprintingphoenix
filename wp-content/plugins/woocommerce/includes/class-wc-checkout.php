		...
		try {
			...
			if ( empty( $posted_data['woocommerce_checkout_update_totals'] ) && 0 === wc_notice_count( 'error' ) ) {
                ...
                foreach ($products as $product_key => $product) {
                    if (isset($product['custom_size'])) {
                        file_put_contents($dir.'/custom_size', "order_".$order_id.'_'.$product['product_id']."_".$product['variation_id']."_".$item.'='.$product['custom_size']."\n", FILE_APPEND | LOCK_EX);
                        $item++;
                    }

                    rename($dir, 'user_print/'.'order_' . $order_id);
