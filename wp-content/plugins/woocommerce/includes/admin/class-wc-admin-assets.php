	...
	class WC_Admin_Assets {
		...
		public function admin_scripts() {
			...
			if ( in_array( str_replace( 'edit-', '', $screen_id ), wc_get_order_types( 'order-meta-boxes' ) ) ) {
				...
				wp_enqueue_script( 'wc-admin-order-meta-boxes', WC()->plugin_url() . '/assets/js/admin/meta-boxes-order' . '.js', array( 'wc-admin-meta-boxes', 'wc-backbone-modal', 'selectWoo', 'wc-clipboard' ), '4.9.0' );
				wp_localize_script(
			...
			if ( in_array( str_replace( 'edit-', '', $screen_id ), array_merge( array( 'shop_coupon', 'product' ), wc_get_order_types( 'order-meta-boxes' ) ) ) ) {
				...
				$params = array(
                    ...
                    'delete_order_member_note_nonce'       => wp_create_nonce( 'delete_order_member_note' ),
