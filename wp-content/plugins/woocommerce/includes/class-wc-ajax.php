...
class WC_AJAX {
	...
	public static function add_ajax_events() {
		...
		$ajax_events = array(
			...
			'load_order_items',
			'add_order_note',
            'add_order_members_chat',
			'delete_order_note',
            'delete_order_member_note',
...
//		check_ajax_referer( 'update-shipping-method', 'security' );
		...
		$view_chosen_shipping_methods = $chosen_shipping_methods;

		$posted_shipping_methods = isset( $_POST['shipping_method'] ) ? wc_clean( wp_unslash( $_POST['shipping_method'] ) ) : array();

		$view_posted_shipping_methods = $posted_shipping_methods;
...
//		if ( wc_ship_to_billing_address_only() ) {
			WC()->customer->set_props(
				array(
					'shipping_country'   => isset( $_POST['s_country'] ) ? wc_clean( wp_unslash( $_POST['s_country'] ) ) : wc_clean( wp_unslash( $_POST['country'] ) ),
					'shipping_state'     => isset( $_POST['s_state'] ) ? wc_clean( wp_unslash( $_POST['s_state'] ) ) : wc_clean( wp_unslash( $_POST['state'] ) ),
					'shipping_postcode'  => isset( $_POST['s_postcode'] ) ? wc_clean( wp_unslash( $_POST['s_postcode'] ) ) : wc_clean( wp_unslash( $_POST['postcode'] ) ),
					'shipping_city'      => isset( $_POST['s_city'] ) ? wc_clean( wp_unslash( $_POST['s_city'] ) ) : wc_clean( wp_unslash( $_POST['city'] ) ),
					'shipping_address_1' => isset( $_POST['s_address'] ) ? wc_clean( wp_unslash( $_POST['s_address'] ) ) : wc_clean( wp_unslash( $_POST['address'] ) ),
					'shipping_address_2' => isset( $_POST['s_address_2'] ) ? wc_clean( wp_unslash( $_POST['s_address_2'] ) ) : wc_clean( wp_unslash( $_POST['address_2'] ) ),
				)
			);
//		} else {
//			WC()->customer->set_props(
...
//		}
     ...
     * Add order note via ajax.
     */
    public static function add_order_members_chat() {
//        check_ajax_referer( 'add-order-members-chat', 'security' );

		if ( ! current_user_can( 'edit_shop_orders' ) || ! isset( $_POST['post_id'], $_POST['message'], $_POST['note_type'] ) ) {
			wp_die( -1 );
		}

		$post_id	= absint( $_POST['post_id'] );
		$message    = wp_kses_post( trim( wp_unslash( $_POST['message'] ) ) );
		$members	= (!empty($_POST['members'])) ? explode(',', $_POST['members']) : null;
		...
		if ( $post_id > 0 ) {
            ...
            foreach ($_FILES['file']['error'] as $key => $error) {
                if ($error == UPLOAD_ERR_OK) {
                    ...
                    $file_hash = explode('.', $name);
                    $file_hash = md5($file_hash[0]) . '.' . $file_hash[1];
                    mkdir('../order_note_files/' . $order);
                    if (move_uploaded_file($tmp_name, '../order_note_files/' . $order . '/' . $file_hash)) {
                        $files[] = $name;
                        $files_hash[] = $file_hash;
                        $attachments[] = $_SERVER['DOCUMENT_ROOT'] . '/order_note_files/' . $order . '/' . $file_hash;
                    };
                }
            }
			...
			global $wpdb;
			$wpdb->get_results("INSERT INTO `wp_codedot_comments` (
                                   `comment_post_ID`,
                                   `comment_author`,
                                   ...
                                   `comment_files_hash`
                                             ...
                                             '".$date."',
                                             '".$message."',
                                             '".json_encode($memberEmail, JSON_UNESCAPED_UNICODE)."',
                                             '".json_encode($files, JSON_UNESCAPED_UNICODE)."',
                                             '".json_encode($files_hash, JSON_UNESCAPED_UNICODE)."'
                                             );"
			);
            $id = $wpdb->insert_id;
			?>
			<li class="note">
                    ...
                    foreach ($files as $k => $file) {
                        ...
                        if ($files_hash) {
                            foreach ($files_hash as $file_hash) {
                                echo '<a href="' . site_url('/order_note_files/' . $order . '/' . $file_hash) . '" download>' . $file . '</a>';
                            }
                        }
                    }
					?>
                ...
                <?php } ?>
                <div class="meta">
                    <abbr class="exact-date"><?php echo $date; ?></abbr>
                    <a data-note-id='<?= $id ?>' class="delete_note">Delete</a>
                </div>
			</li>
			<?php
		}
	 ...
	 */
    public static function delete_order_member_note() {
        ...
        if ( $note_id > 0 ) {
            $wpdb->delete('wp_codedot_comments', ['comment_ID' => $note_id]);
        }
        wp_die();
    }
