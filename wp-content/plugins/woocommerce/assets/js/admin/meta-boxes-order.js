...
jQuery( function ( $ ) {
	 ...
	 * Order Members Chat
	 */
	var wc_meta_boxes_order_members_chat = {
		init: function () {
			$('#woocommerce-order-members-chat')
				.on('click', 'button.add_members_chat', this.add_order_members_chat)
				.on('click', 'a.delete_member_note', this.delete_order_note);

		},

		add_order_members_chat: function() {
			let form_data = new FormData();
			form_data.append('message', $('#woocommerce-order-members-chat textarea#messages').val());
			form_data.append('members', $('#woocommerce-order-members-chat select#members').val());

			form_data.append('action', 'woocommerce_add_order_members_chat');
			form_data.append('post_id', woocommerce_admin_meta_boxes.post_id);
			form_data.append('note_type', '');
			form_data.append('security', woocommerce_admin_meta_boxes.add_order_note_nonce);

			let file_data = $("#woocommerce-order-members-chat input#documents").prop('files');
			$.each(file_data,function(key, file){
			...
			$.ajax({
				...
				type: 'post',
				success: function(response){
					$( 'ul.order_members_chat' ).prepend( response );
				}
			});
		...
		delete_order_note: function() {
			if ( window.confirm( woocommerce_admin_meta_boxes.i18n_delete_note ) ) {
				var note = $( this ).closest( 'li.note' );
				let note_id = $(note).children('div.meta').children('a.delete_member_note').data('note-id')
				...
				var data = {
					action:   'woocommerce_delete_order_member_note',
					note_id:  note_id,
					security: woocommerce_admin_meta_boxes.delete_order_member_note_nonce
				};
	...
	wc_meta_boxes_order_members_chat.init();
